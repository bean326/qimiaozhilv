...
/*		SaverChanger.isNewSave = false;//增加 为 false  减少为true
		list = databaseAccessor.loadPlayers();
		SaverChanger.isNewSave = true;
		databaseAccessor.savePlayers(list);
...
//		databaseAccessor.savePlayers(list);
	...
	private void doAnswerPlayers(List list)
	{
		int size = list.size();//清除答题数据
		...
		GameServer.getInstance().getWorldManager().getAnswerRewards().clear();
	...
	{	
		...
		//
		//游戏中国  4567区       送8小时3倍经验卡1张，500万精力生命宝石各1个
		for (int j = 0; j < size; j++)
		{
			Player player = (Player)list.get(j);
			...
			Goods [] goods = DataFactory.getInstance().makeGoods(1045000027,1);
			if(goods != null)
			{
				mail.addAttach(goods[0]);
			}
			
			goods = DataFactory.getInstance().makeGoods(1045000185,1);
			if(goods != null)
			{
				mail.addAttach(goods[0]);
			}
			...
			mail = new Mail("\u6E38\u620F\u7BA1\u7406\u5458",System.currentTimeMillis());
			mail.setTitle("\u7CFB\u7EDF\u90AE\u4EF6");//系统邮件
			
			
			goods = DataFactory.getInstance().makeGoods(1045000183,1);
			if(goods != null)
			{
				mail.addAttach(goods[0]);
			}
