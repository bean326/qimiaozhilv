...
public class SkillTome extends PlayerExtInfo
{
	...
	public void writeTo(ByteBuffer buffer)
	{
		...
		for (int i = 0; i < skills.length; i++)
		{
			...
			buffer.writeByte(skills[i].type);
	...
	public List getProfessionSkills(Player player)
	{
		...
		for (int i = 0; i < size; i++)
		{
			...
			if(skill instanceof ActiveSkill)
			{
				...
				if(as.profession[0] == Monster.MONSTERSKILLTYPE)
					continue;
...
//							isPubSkill = false;
					...
					{
						int jobType = player.getZhuanshengState();
					
						if(jobType > 0)//转生后
						{
							if(as.profession[j] == player.upProfession + 4)
							{
								as.isPubSkill = isPubSkill;
								if(!skillList.contains(as))
									skillList.add(as);
			...
			else if(skill instanceof PassiveSkill)
			{
				...
				if(pSkill.profession[0] == Monster.MONSTERSKILLTYPE)
					continue;
...
//							isPubSkill = false;
					...
					else
					{
						...
						if(jobType > 0)//转生后
						{
							if(pSkill.profession[j] == player.upProfession + 4)
							{
								pSkill.isPubSkill = isPubSkill;
								if(!skillList.contains(pSkill))
									skillList.add(pSkill);
...
//				System.out.println(skills[i].name+"//////"+skills[i].level+"///////"+skills[i].isStudied);
	...
	{
		skills = null;
		activeSkills = new ActiveSkill[0];
		passiveSkills = new PassiveSkill[0];
	}
}
